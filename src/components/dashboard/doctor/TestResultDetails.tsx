...
    summary: [
        '3-Hydroxy-3-methylglutaric acidemia (HMG acidemia) là một rối loạn chuyển hóa bẩm sinh hiếm gặp do đột biến gen HMGCL, gây thiếu hụt enzym 3-hydroxy-3-methylglutaryl-CoA lyase. Enzym này cần thiết cho quá trình phân giải leucine và sản xuất thể ceton trong thời kỳ nhịn đói hoặc stress chuyển hóa.',
...
    overview: {
      signsAndSymptoms: {
        earlyStage: ['Khởi phát đột ngột khi nhịn ăn, nhiễm trùng hoặc stress chuyển hóa', 'Hạ đường huyết nghiêm trọng nhưng không có thể ceton (hypoketotic hypoglycemia)', 'Nôn mửa, lừ đừ, giảm trương lực cơ, co giật, hôn mê', 'Nhiễm toan chuyển hóa, tăng amoniac, tăng lactate'],
        lateStage: ['Trẻ có thể chậm phát triển, có nguy cơ tổn thương thần kinh', 'Biểu hiện thần kinh mạn tính như co giật, giảm trương lực cơ, loạn trương lực'],
        general: ['Triệu chứng thường xuất hiện trong 1 năm đầu đời và các đợt cấp có thể tái phát nếu không được phòng ngừa hiệu quả.']
      },
      causes: [
...
      treatmentDetails: {
        prevention: ['Tránh nhịn đói: ăn thường xuyên, cả ngày và đêm', 'Tăng năng lượng qua glucose khi bệnh', 'Tránh nhiễm trùng và các yếu tố gây stress chuyển hóa'],
        diet: ['Hạn chế leucine trong chế độ ăn (protein vừa phải)', 'Bổ sung thực phẩm y tế chuyên biệt ít leucine (theo chỉ định chuyên gia dinh dưỡng)', 'Bổ sung glucose hoặc maltodextrin để duy trì năng lượng'],
        acuteTreatment: ['Truyền glucose, có thể phối hợp insulin', 'Theo dõi và điều chỉnh điện giải, acid-base', 'Kiểm soát hạ đường huyết, tăng amoniac'],
        monitoring: ['Đánh giá sự phát triển thể chất, tâm thần vận động', 'Xét nghiệm chức năng gan, điện giải, lactate, amoniac']
      },
...
    name: 'ARGININEMIA',
    abbreviation: 'ARG',
    classification: 'Bệnh rối loạn chuyển hóa Acid amin',
    ...
    confirmatoryDiagnosis: ['Xét nghiệm enzym arginase: xác định hoạt tính thấp trong hồng cầu', 'Xét nghiệm gen: phân tích gen ARG1 để xác định đột biến'],
...
    summary: [
        'Argininemia là một rối loạn chuyển hóa hiếm gặp, thuộc nhóm rối loạn chu trình ure. Bệnh xảy ra do đột biến ở gen ARG1, gây thiếu hụt enzym arginase 1 – enzym cần thiết để chuyển hóa arginine thành ornithine và urê nhằm thải trừ nitơ dư thừa ra khỏi cơ thể.',
...
      diagnosticMethods: [
        'Sàng lọc sơ sinh: Argininemia có thể được phát hiện qua chương trình sàng lọc sơ sinh bằng khối phổ ghép nối (MS/MS) – phát hiện mức arginine tăng cao trong máu.',
...
      treatmentDetails: {
        diet: ['Chế độ ăn ít protein, đặc biệt là hạn chế arginine', 'Sử dụng các công thức y tế chuyên biệt không chứa arginine', 'Hỗ trợ bởi chuyên gia dinh dưỡng chuyên sâu về rối loạn chuyển hóa'],
        acuteTreatment: ['Sodium benzoate, sodium phenylbutyrate hoặc glycerol phenylbutyrate có thể được sử dụng để giúp thải amoniac qua con đường thay thế'],
        monitoring: ['Đánh giá sự phát triển vận động và thần kinh', 'Theo dõi nồng độ arginine, ammonia và các chỉ số dinh dưỡng'],
        prevention: ['Vật lý trị liệu để hỗ trợ vận động', 'Thuốc chống co giật nếu cần']
      },
...
    classification: 'Bệnh rối loạn chuyển hóa Acid amin',
    ...
    confirmatoryDiagnosis: ['Xét nghiệm enzym: đo hoạt tính argininosuccinate synthetase trong nguyên bào sợi da', 'Xét nghiệm gen: xác định đột biến trong gen ASS1'],
...
    treatment: 'Chế độ ăn hạn chế protein; truyền glucose; sodium benzoate/phenylbutyrate; lọc máu khi cần',
    summary: [
        'Citrullinemia type I (CTLN1) là một rối loạn chuyển hóa bẩm sinh hiếm gặp thuộc nhóm rối loạn chu trình ure – con đường chính của cơ thể để loại bỏ amoniac dư thừa. Bệnh do đột biến ở gen ASS1, gây thiếu hụt enzym argininosuccinate synthetase. Khi enzym này bị thiếu hoặc bất hoạt, amoniac tích tụ trong máu (tăng ammoniac máu), dẫn đến tổn thương thần kinh nghiêm trọng hoặc tử vong nếu không được điều trị kịp thời.',
...
    overview: {
      signsAndSymptoms: {
        earlyStage: ['Khởi phát trong vài ngày đầu sau sinh', 'Bú kém, nôn mửa', 'Ngủ gà, giảm trương lực cơ', 'Co giật, hôn mê', 'Tăng ammoniac máu nặng (thường > 1000 µmol/L)', 'Nếu không điều trị, tổn thương não không hồi phục hoặc tử vong'],
        lateStage: ['Triệu chứng xuất hiện ở trẻ nhỏ hoặc thanh thiếu niên', 'Có thể gồm: đau đầu, rối loạn hành vi, giảm tập trung, buồn nôn, giảm lực cơ', 'Một số người mang đột biến nhưng không có triệu chứng'],
        general: ['Tăng nồng độ citrulline trong huyết tương', 'Tăng ammoniac máu', 'Nồng độ acid amin bất thường trong huyết tương và nước tiểu']
      },
      causes: [
...
      diagnosticMethods: [
        'Sàng lọc sơ sinh: Được phát hiện thông qua chương trình MS/MS – nồng độ citrulline tăng cao trong mẫu máu gót chân',
        'Xét nghiệm chẩn đoán: ',
...
        acuteTreatment: [
        ...
        diet: ['Chế độ ăn hạn chế protein (tùy theo độ tuổi và mức độ nặng)', 'Sử dụng thực phẩm y tế chuyên biệt (không chứa citrulline)', 'Bổ sung arginine', 'Thuốc hỗ trợ thải amoniac'],
        monitoring: ['Xét nghiệm ammoniac, acid amin máu', 'Đánh giá phát triển thần kinh, vận động', 'Đánh giá chế độ dinh dưỡng']
      },
...
    classification: 'Bệnh lý axit hữu cơ',
    ...
    confirmatoryDiagnosis: ['Xét nghiệm DNA', 'Đo hoạt độ enzyme isovaleryl-CoA dehydrogenase'],
...
    diagnosis: 'MS/MS: tăng glutarylcarnitine (C5DC); acid hữu cơ (glutaric, 3-hydroxyglutaric)',
    treatment: 'Chế độ ăn hạn chế lysine/tryptophan; bổ sung L-carnitine',
    summary: [
        'Glutaric acidemia type I (GA-I) là một rối loạn chuyển hóa hiếm gặp do đột biến ở gen GCDH, dẫn đến thiếu hụt hoặc mất chức năng enzym glutaryl-CoA dehydrogenase. Enzym này có vai trò phân hủy các acid amin lysine, hydroxylysine và tryptophan. Khi thiếu enzym, các acid hữu cơ độc hại (glutaric acid và 3-hydroxyglutaric acid) tích tụ trong cơ thể, đặc biệt là trong não, gây tổn thương thần kinh nghiêm trọng.',
...
    overview: {
      signsAndSymptoms: {
        general: ['GA-I có phổ biểu hiện lâm sàng rộng, từ thể không triệu chứng đến tổn thương thần kinh nghiêm trọng. Đặc điểm chung bao gồm:'],
        earlyStage: [
        ...
        lateStage: ['Một số trường hợp chỉ biểu hiện chậm phát triển trí tuệ nhẹ, rối loạn ngôn ngữ hoặc vận động tinh.'],
        special: ['Tăng chu vi đầu (macrocephaly) thường là dấu hiệu sớm nhất nhưng dễ bị bỏ qua. Có thể phát hiện máu trong nước tiểu, tăng acid hữu cơ, đặc biệt là acid glutaric và 3-hydroxyglutaric.']
      },
      causes: [
...
      treatmentDetails: {
        prevention: ['Mục tiêu điều trị là ngăn ngừa hoặc làm giảm nguy cơ tổn thương não.'],
        diet: ['Hạn chế lysine và tryptophan', 'Bổ sung L-carnitine để tăng thải độc', 'Sử dụng công thức dinh dưỡng chuyên biệt không chứa lysine'],
        acuteTreatment: [
              ...
              {disease && disease.classification ? (
                <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-5 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-slate-900">{disease.abbreviation || 'N/A'}</p>
                  </div>

                  <div>
                    <h4 className="font-semibold text-slate-600">Tỷ lệ mắc bệnh</h4>
                    <p className="text-slate-900">{disease.incidenceRate || 'N/A'}</p>
                  </div>
                  ...
                  <div className="md:col-span-2">
                    ...
                    <p className="text-slate-900">{disease.geneticMechanism || 'N/A'}</p>
                  </div>
...
                          <div className="space-y-3 prose prose-sm max-w-none">
                              {Array.isArray(disease.overview.signsAndSymptoms.general) ? disease.overview.signsAndSymptoms.general.map((p: string, i: number) => <p key={i}>{p}</p>) : <p>{disease.overview.signsAndSymptoms.general}</p>}
...
                            {disease.overview.signsAndSymptoms.lateStage && (<div className="mt-2"><h4 className="font-semibold">Giai đoạn muộn:</h4><ul className="list-disc list-inside pl-4">{disease.overview.signsAndSymptoms.lateStage.map((s: string, i: number) => <li key={i}>{s}</li>)}</ul></div>)}
                              {disease.overview.signsAndSymptoms.special && (<div className="mt-2"><h4 className="font-semibold">Đặc điểm khác:</h4><p>{disease.overview.signsAndSymptoms.special}</p></div>)}
                          </div>
                        </div>
                      )}
                      {disease.overview.causes && (<div><h3 className="text-lg font-semibold mb-2 border-b pb-1">Nguyên nhân</h3><div className="space-y-2 prose prose-sm max-w-none">{Array.isArray(disease.overview.causes) ? disease.overview.causes.map((p: string, i: number) => <p key={i}>{p}</p>) : <p>{disease.overview.causes}</p>}</div></div>)}
...
                      </div>
                      {disease.overview.diagnosticMethods && (<div><h3 className="text-lg font-semibold mb-2 border-b pb-1">Phương pháp chẩn đoán</h3><div className="space-y-2 prose prose-sm max-w-none">{Array.isArray(disease.overview.diagnosticMethods) ? disease.overview.diagnosticMethods.map((p: string, i: number) => <p key={i}>{p}</p>) : <p>{disease.overview.diagnosticMethods}</p>}</div></div>)}
